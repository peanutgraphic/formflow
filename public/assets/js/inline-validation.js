...
(function($, window) {
    ...
    var ISFInlineValidation = {
        ...
        bindEvents: function() {
            ...
            if (this.config.validateOnBlur) {
                $(document).on('blur', '.isf-input, .isf-select', function() {
            ...
            if (this.config.validateOnKeyup) {
                $(document).on('keyup', '.isf-input', function() {
                    var $field = $(this);
            ...
            $(document).on('focus', '.isf-input, .isf-select', function() {
                var $field = $(this);
            ...
            $(document).on('isf:stepLoaded', function() {
        ...
        initializeExistingFields: function() {
            ...
            $('.isf-step-form:visible .isf-input[required], .isf-step-form:visible .isf-select[required]').each(function() {
                var $field = $(this);
        ...
        setFieldSuccess: function($field) {
            ...
            var $wrapper = $field.closest('.isf-field');
            $field.addClass('isf-input-valid').removeClass('isf-input-error');

            // Add success icon if not exists
            if (!$wrapper.find('.isf-validation-icon').length) {
                var successIcon = '<span class="isf-validation-icon isf-validation-success" aria-hidden="true">' +
                    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" width="18" height="18">' +
                    '<path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>' +
                    '</svg></span>';

                // For input groups, add after the group
                var $inputGroup = $field.closest('.isf-input-group');
                if ($inputGroup.length) {
                    $inputGroup.after(successIcon);
                } else {
                    $field.after(successIcon);
                }
            }
        },
        ...
        setFieldError: function($field, message) {
            var $wrapper = $field.closest('.isf-field');
            $field.addClass('isf-input-error').removeClass('isf-input-valid');

            // Remove any success icon
            $wrapper.find('.isf-validation-icon').remove();

            // Add error message if not exists
            if (!$wrapper.find('.isf-field-error').length && message) {
                var errorEl = '<span class="isf-field-error" role="alert">' + this.escapeHtml(message) + '</span>';
        ...
        clearFieldState: function($field) {
            var $wrapper = $field.closest('.isf-field');
            $field.removeClass('isf-input-valid isf-input-error');
            $wrapper.find('.isf-field-error').remove();
            $wrapper.find('.isf-validation-icon').remove();
        },
        ...
        validateForm: function($form) {
            ...
            $form.find('.isf-input[required], .isf-select[required]').each(function() {
                var $field = $(this);
        ...
        getMessage: function(key) {
            if (typeof isf_frontend !== 'undefined' && isf_frontend.strings && isf_frontend.strings[key]) {
                return isf_frontend.strings[key];
            }
    ...
    // Export to window
    window.ISFInlineValidation = ISFInlineValidation;

    // Auto-initialize when DOM ready if config available
    $(document).ready(function() {
        if (typeof isf_frontend !== 'undefined' && isf_frontend.features) {
            var validationConfig = isf_frontend.features.inline_validation || {};
            ISFInlineValidation.init({
