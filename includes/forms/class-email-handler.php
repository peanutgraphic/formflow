...
class EmailHandler
{
    ...
    private function getCustomerSubject(array $form_data): string
    {
        return sprintf(
            ...
            __('%s Energy Wise Rewards - Enrollment Confirmation', 'formflow'),
            $this->getUtilityName()
        );
    }
    ...
    private function getAdminSubject(array $form_data, string $confirmation_number): string
    {
        return sprintf(
            ...
            __('New Enrollment: %1$s - %2$s', 'formflow'),
            $confirmation_number,
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: #f8f9fa; padding: 30px; border: 1px solid #ddd; border-top: none;">
                <h2 style="color: #28a745; margin-top: 0;">
                    <?php esc_html_e('Enrollment Confirmed!', 'formflow'); ?>
                </h2>

                <p><?php printf(
                    /* translators: %s: Customer first name */
                    esc_html__('Dear %s,', 'formflow'),
                    esc_html($form_data['first_name'] ?? '')
                ); ?></p>

                <p><?php esc_html_e('Thank you for enrolling in the Energy Wise Rewards program. Your installation appointment has been scheduled.', 'formflow'); ?></p>

                <div style="background: #d4edda; border: 1px solid #28a745; border-radius: 4px; padding: 16px; margin: 20px 0; text-align: center;">
                    <p style="margin: 0; font-size: 12px; color: #666; text-transform: uppercase;"><?php esc_html_e('Confirmation Number', 'formflow'); ?></p>
                ...
                <h3 style="border-bottom: 1px solid #ddd; padding-bottom: 10px;"><?php esc_html_e('Appointment Details', 'formflow'); ?></h3>

                <table style="width: 100%; border-collapse: collapse;">
                    <tr>
                        <td style="padding: 8px 0; color: #666;"><?php esc_html_e('Device:', 'formflow'); ?></td>
                        <td style="padding: 8px 0; font-weight: bold;"><?php echo esc_html($device_name); ?></td>
                    </tr>
                    <tr>
                        <td style="padding: 8px 0; color: #666;"><?php esc_html_e('Date:', 'formflow'); ?></td>
                        <td style="padding: 8px 0; font-weight: bold;"><?php echo esc_html($date); ?></td>
                    </tr>
                    <tr>
                        <td style="padding: 8px 0; color: #666;"><?php esc_html_e('Time:', 'formflow'); ?></td>
                        <td style="padding: 8px 0; font-weight: bold;"><?php echo esc_html($time); ?></td>
                    </tr>
                    <tr>
                        <td style="padding: 8px 0; color: #666;"><?php esc_html_e('Address:', 'formflow'); ?></td>
                ...
                <h3 style="border-bottom: 1px solid #ddd; padding-bottom: 10px; margin-top: 30px;"><?php esc_html_e('What to Expect', 'formflow'); ?></h3>

                <ul style="padding-left: 20px;">
                    <li><?php esc_html_e('A certified technician will arrive during your scheduled time window', 'formflow'); ?></li>
                    <li><?php esc_html_e('Please ensure an adult (18+) is present for the installation', 'formflow'); ?></li>
                    <li><?php esc_html_e('Installation is FREE and typically takes 30-45 minutes', 'formflow'); ?></li>
                    <li><?php esc_html_e('You may receive a confirmation call 1-2 days before your appointment', 'formflow'); ?></li>
                </ul>

                <?php if (!empty($form_data['special_instructions'])) : ?>
                <p><strong><?php esc_html_e('Your Special Instructions:', 'formflow'); ?></strong><br>
                <?php echo esc_html($form_data['special_instructions']); ?></p>
                <?php endif; ?>

                <p style="margin-top: 30px;"><?php esc_html_e('If you need to reschedule or have any questions, please contact us.', 'formflow'); ?></p>

                <p><?php esc_html_e('Thank you for helping save energy!', 'formflow'); ?></p>
            ...
            <div style="background: #333; color: #999; padding: 20px; text-align: center; font-size: 12px; border-radius: 0 0 8px 8px;">
                <p style="margin: 0;"><?php esc_html_e('This is an automated message. Please do not reply directly to this email.', 'formflow'); ?></p>
            </div>
        </body>
        </html>
        <?php
    ...
    private function getAdminEmailBody(array $form_data, string $confirmation_number): string
    {
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
            <h2 style="color: #0066cc; margin-top: 0;"><?php esc_html_e('New Energy Wise Rewards Enrollment', 'formflow'); ?></h2>

            <p><strong><?php esc_html_e('Confirmation Number:', 'formflow'); ?></strong> <?php echo esc_html($confirmation_number); ?></p>
            <p><strong><?php esc_html_e('Instance:', 'formflow'); ?></strong> <?php echo esc_html($this->instance['name'] ?? ''); ?></p>

            <h3 style="border-bottom: 1px solid #ddd; padding-bottom: 10px;"><?php esc_html_e('Customer Information', 'formflow'); ?></h3>

            <table style="width: 100%; border-collapse: collapse;">
                <tr>
                    <td style="padding: 6px 0; color: #666; width: 150px;"><?php esc_html_e('Name:', 'formflow'); ?></td>
                    <td style="padding: 6px 0;"><?php echo esc_html(($form_data['first_name'] ?? '') . ' ' . ($form_data['last_name'] ?? '')); ?></td>
                </tr>
                <tr>
                    <td style="padding: 6px 0; color: #666;"><?php esc_html_e('Email:', 'formflow'); ?></td>
                    <td style="padding: 6px 0;"><a href="mailto:<?php echo esc_attr($form_data['email'] ?? ''); ?>"><?php echo esc_html($form_data['email'] ?? ''); ?></a></td>
                </tr>
                <tr>
                    <td style="padding: 6px 0; color: #666;"><?php esc_html_e('Phone:', 'formflow'); ?></td>
                ...
                <tr>
                    <td style="padding: 6px 0; color: #666;"><?php esc_html_e('Alt Phone:', 'formflow'); ?></td>
                    <td style="padding: 6px 0;"><?php echo esc_html($form_data['alt_phone']); ?> (<?php echo esc_html($form_data['alt_phone_type'] ?? ''); ?>)</td>
                </tr>
                <?php endif; ?>
                <tr>
                    <td style="padding: 6px 0; color: #666;"><?php esc_html_e('Account Number:', 'formflow'); ?></td>
                    <td style="padding: 6px 0;"><?php echo esc_html($form_data['utility_no'] ?? ''); ?></td>
                </tr>
            </table>

            <h3 style="border-bottom: 1px solid #ddd; padding-bottom: 10px; margin-top: 20px;"><?php esc_html_e('Service Address', 'formflow'); ?></h3>
            ...
            <h3 style="border-bottom: 1px solid #ddd; padding-bottom: 10px; margin-top: 20px;"><?php esc_html_e('Appointment Details', 'formflow'); ?></h3>

            <table style="width: 100%; border-collapse: collapse;">
                <tr>
                    <td style="padding: 6px 0; color: #666; width: 150px;"><?php esc_html_e('Device:', 'formflow'); ?></td>
                    <td style="padding: 6px 0;"><?php echo esc_html($device_name); ?></td>
                </tr>
                <tr>
                    <td style="padding: 6px 0; color: #666;"><?php esc_html_e('Date:', 'formflow'); ?></td>
                    <td style="padding: 6px 0;"><?php echo esc_html($date); ?></td>
                </tr>
                <tr>
                    <td style="padding: 6px 0; color: #666;"><?php esc_html_e('Time:', 'formflow'); ?></td>
                    <td style="padding: 6px 0;"><?php echo esc_html($time); ?></td>
                </tr>
            </table>

            <?php if (!empty($form_data['special_instructions'])) : ?>
            <h3 style="border-bottom: 1px solid #ddd; padding-bottom: 10px; margin-top: 20px;"><?php esc_html_e('Special Instructions', 'formflow'); ?></h3>
            <p><?php echo esc_html($form_data['special_instructions']); ?></p>
            <?php endif; ?>

            <?php if (!empty($form_data['promo_code'])) : ?>
            <p><strong><?php esc_html_e('Promo Code:', 'formflow'); ?></strong> <?php echo esc_html($form_data['promo_code']); ?></p>
            <?php endif; ?>

            <hr style="margin: 30px 0; border: none; border-top: 1px solid #ddd;">

            <p style="font-size: 12px; color: #666;">
                <?php esc_html_e('Submitted:', 'formflow'); ?> <?php echo esc_html(current_time('mysql')); ?><br>
                <?php esc_html_e('IP Address:', 'formflow'); ?> <?php echo esc_html($_SERVER['REMOTE_ADDR'] ?? 'Unknown'); ?>
            </p>
        </body>
        </html>
        <?php
    ...
    {
        $slots = [
            'AM' => __('Morning (8:00 AM - 12:00 PM)', 'formflow'),
            'MD' => __('Midday (10:00 AM - 2:00 PM)', 'formflow'),
            'PM' => __('Afternoon (12:00 PM - 5:00 PM)', 'formflow'),
            'EV' => __('Evening (3:00 PM - 7:00 PM)', 'formflow'),
        ];
