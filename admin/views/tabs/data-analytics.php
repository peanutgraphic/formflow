...
<div class="isf-analytics" id="isf-analytics-content">
    ...
    <div class="isf-charts-section">
        ...
        <div class="isf-chart-container">
            <h2><?php esc_html_e('Submission Trends Over Time', 'formflow'); ?></h2>
            <p class="isf-chart-description"><?php esc_html_e('Daily submission activity showing started vs completed forms.', 'formflow'); ?></p>
            <?php if (!empty($daily)) : ?>
            <canvas id="isf-trends-chart" height="80"></canvas>
            <?php else : ?>
            <p class="isf-no-data"><?php esc_html_e('No trend data available.', 'formflow'); ?></p>
            <?php endif; ?>
        </div>

        <!-- Field Completion Rates Chart -->
        <div class="isf-chart-container">
            <h2><?php esc_html_e('Field Completion Rates', 'formflow'); ?></h2>
            <p class="isf-chart-description"><?php esc_html_e('Percentage of users who completed each field.', 'formflow'); ?></p>
            <?php if (!empty($funnel)) : ?>
            <canvas id="isf-completion-chart" height="80"></canvas>
            <?php else : ?>
            <p class="isf-no-data"><?php esc_html_e('No completion data available.', 'formflow'); ?></p>
            <?php endif; ?>
        </div>

        <!-- Drop-off Points Chart -->
        <div class="isf-chart-container">
            <h2><?php esc_html_e('Drop-off Points Analysis', 'formflow'); ?></h2>
            <p class="isf-chart-description"><?php esc_html_e('Identify where users abandon the form.', 'formflow'); ?></p>
            <?php if (!empty($funnel)) : ?>
            <canvas id="isf-dropoff-chart" height="80"></canvas>
            <?php else : ?>
...
<?php if (!empty($daily) || !empty($funnel)) : ?>
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<script>
jQuery(document).ready(function($) {
    // Submission Trends Line Chart
    <?php if (!empty($daily)) : ?>
    var trendsCtx = document.getElementById('isf-trends-chart');
    if (trendsCtx) {
        new Chart(trendsCtx, {
            type: 'line',
            data: {
                ...
                datasets: [{
                    ...
                    label: '<?php echo esc_js(__('Abandoned', 'formflow')); ?>',
                    data: <?php echo json_encode(array_map(function($row) {
                        return ($row['started'] ?? 0) - ($row['completed'] ?? 0);
                    }, $daily)); ?>,
                    borderColor: '#ea4335',
    ...
    // Field Completion Rates Bar Chart
    <?php if (!empty($funnel)) : ?>
    var completionCtx = document.getElementById('isf-completion-chart');
    if (completionCtx) {
        new Chart(completionCtx, {
            type: 'bar',
            data: {
                labels: <?php echo json_encode(array_map(function($step) {
                    return $step['step_name'] ?? 'Step ' . $step['step'];
                }, $funnel)); ?>,
                datasets: [{
                    label: '<?php echo esc_js(__('Completion Rate', 'formflow')); ?>',
                    data: <?php echo json_encode(array_map(function($step) {
                        $total = $step['sessions_entered'] ?? 1;
                        $completed = $total - ($step['drop_off_count'] ?? 0);
    ...
    // Drop-off Points Chart
    var dropoffCtx = document.getElementById('isf-dropoff-chart');
    if (dropoffCtx) {
        new Chart(dropoffCtx, {
            ...
            data: {
                ...
                datasets: [{
                    label: '<?php echo esc_js(__('Drop-off Rate', 'formflow')); ?>',
                    data: <?php echo json_encode(array_column($funnel, 'drop_off_rate')); ?>,
                    backgroundColor: function(context) {
                        const value = context.parsed.y;
                        if (value > 30) return '#ea4335';
                        if (value > 15) return '#fbbc04';
                        return '#34a853';
                    },
                    borderWidth: 0,
                    borderRadius: 4
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                return context.parsed.x + '% dropped off';
                            }
                        }
                    }
                },
    ...
    // Old daily chart for backwards compatibility
