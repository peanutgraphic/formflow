...
<table class="form-table isf-feature-config-table">
    <tr>
        <th scope="row"><?php esc_html_e('Allowed Actions', 'formflow'); ?></th>
        <td>
            <fieldset>
                <label class="isf-checkbox-label">
                    ...
                    <?php esc_html_e('Allow customers to reschedule appointments', 'formflow'); ?>
                </label>
                <br>
                <label class="isf-checkbox-label">
                    ...
                    <?php esc_html_e('Allow customers to cancel appointments', 'formflow'); ?>
                </label>
            </fieldset>
        </td>
    </tr>
    <tr>
        <th scope="row">
            <label for="reschedule_deadline"><?php esc_html_e('Reschedule Deadline', 'formflow'); ?></label>
        </th>
        <td>
            <select id="reschedule_deadline" name="settings[features][appointment_self_service][reschedule_deadline_hours]">
                ...
                <option value="72" <?php selected($settings['reschedule_deadline_hours'] ?? 24, 72); ?>>
                    <?php esc_html_e('72 hours before appointment', 'formflow'); ?>
                </option>
            </select>
            <p class="description"><?php esc_html_e('How far in advance customers must reschedule', 'formflow'); ?></p>
        </td>
    </tr>
    <tr>
        <th scope="row">
            <label for="cancel_deadline"><?php esc_html_e('Cancellation Deadline', 'formflow'); ?></label>
        </th>
        <td>
            ...
            <p class="description"><?php esc_html_e('How far in advance customers must cancel', 'formflow'); ?></p>
        </td>
    </tr>
    <tr>
        <th scope="row"><?php esc_html_e('Cancellation Reason', 'formflow'); ?></th>
        <td>
            <label class="isf-checkbox-label">
                ...
                <?php esc_html_e('Require customers to provide a reason when cancelling', 'formflow'); ?>
            </label>
        </td>
    </tr>
    <tr>
        <th scope="row">
            <label for="token_expiry"><?php esc_html_e('Link Expiry', 'formflow'); ?></label>
        </th>
        <td>
            <select id="token_expiry" name="settings[features][appointment_self_service][token_expiry_days]">
                <option value="7" <?php selected($settings['token_expiry_days'] ?? 30, 7); ?>>
                    <?php esc_html_e('7 days', 'formflow'); ?>
                </option>
                <option value="14" <?php selected($settings['token_expiry_days'] ?? 30, 14); ?>>
                    <?php esc_html_e('14 days', 'formflow'); ?>
                </option>
                <option value="30" <?php selected($settings['token_expiry_days'] ?? 30, 30); ?>>
                    <?php esc_html_e('30 days', 'formflow'); ?>
                </option>
                <option value="60" <?php selected($settings['token_expiry_days'] ?? 30, 60); ?>>
                    <?php esc_html_e('60 days', 'formflow'); ?>
                </option>
            </select>
            <p class="description"><?php esc_html_e('How long the management link in confirmation emails remains valid', 'formflow'); ?></p>
        </td>
    </tr>
</table>

<div class="isf-info-box">
    <p><strong><?php esc_html_e('How It Works:', 'formflow'); ?></strong></p>
    <p><?php esc_html_e('After enrollment, customers receive a confirmation email with a secure link to manage their appointment. They can reschedule or cancel within the configured deadlines. All changes are logged and notification emails are sent.', 'formflow'); ?></p>
</div>
