...
(function() {
    ...
    const ISFSecurity = {
        ...
        protectConsole: function() {
            ...
            //     if (e.target.closest('.isf-form-container')) {
            //         e.preventDefault();
            //     }
            // });
        },
        ...
        addTimingProtection: function() {
            ...
            document.querySelectorAll('.isf-form-container form').forEach(function(form) {
                const loadTimeInput = document.createElement('input');
                loadTimeInput.type = 'hidden';
                loadTimeInput.name = 'isf_form_load_time';
                ...
                form.addEventListener('submit', function() {
                    ...
                    scoreInput.name = 'isf_interaction_score';
        ...
        protectFormData: function() {
            ...
            document.querySelectorAll('.isf-form-container input').forEach(function(input) {
        ...
        addRequestSigning: function() {
            ...
            function SignedXHR() {
                ...
                xhr.send = function(data) {
                    ...
                    if (this._url && this._url.indexOf('isf_') !== -1) {
                        const signature = self.generateRequestSignature(this._url, data);
                        this.setRequestHeader('X-ISF-Signature', signature);
                        this.setRequestHeader('X-ISF-Timestamp', Date.now().toString());
                        this.setRequestHeader('X-ISF-Score', self.getInteractionScore().toString());
                    }
            ...
            window.fetch = function(url, options) {
                ...
                // Only sign our own requests
                if (typeof url === 'string' && url.indexOf('isf_') !== -1) {
                    options.headers = options.headers || {};
                    options.headers['X-ISF-Signature'] = self.generateRequestSignature(url, options.body);
                    options.headers['X-ISF-Timestamp'] = Date.now().toString();
                    options.headers['X-ISF-Score'] = self.getInteractionScore().toString();
                }
    ...
    window.ISFSecurity = ISFSecurity;

    // Auto-initialize when DOM is ready
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function() {
            if (window.isfSecurityConfig) {
                ISFSecurity.init(window.isfSecurityConfig);
            }
        });
    } else {
        if (window.isfSecurityConfig) {
