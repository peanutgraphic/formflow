...
class CacheManager {
     ...
     * Cache group name
     */
    public const GROUP = 'isf_cache';

    /**
     * Cache key prefix
     */
    public const PREFIX = 'isf_';
    ...
    private function init_redis(): bool {
        ...
        $host = defined('ISF_REDIS_HOST') ? ISF_REDIS_HOST : '127.0.0.1';
        $port = defined('ISF_REDIS_PORT') ? ISF_REDIS_PORT : 6379;
        $password = defined('ISF_REDIS_PASSWORD') ? ISF_REDIS_PASSWORD : null;
        $database = defined('ISF_REDIS_DATABASE') ? ISF_REDIS_DATABASE : 0;

        // Check if Redis is enabled for this plugin
        if (defined('ISF_DISABLE_REDIS') && ISF_DISABLE_REDIS) {
            return false;
        }
 ...
 * @return CacheManager
 */
function isf_cache(): CacheManager {
